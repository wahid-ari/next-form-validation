...
test('test Yup default valid value and click Submit', async ({ page }) => {
  await page.goto('/');
...
test('test Yup default invalid value and click Submit', async ({ page }) => {
  await page.goto('/');
  await expect(page).toHaveURL('/');
  ...
  await expect(page.locator('p:has-text("DOB must be date")')).toBeVisible();
